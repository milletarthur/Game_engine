...
public class Field2 {
	 ...
	 * ArrayList<Object> lab) { ArrayList<LinkedList<Entity>> row =
	 * (ArrayList<LinkedList<Entity>>) lab.get(indice_i); LinkedList<Entity> elem =
	 * row.get(indice_j); elem.addFirst(e); }
	 * 
	 * public Entity get_element(int indice_i, int indice_j, ArrayList<Object> lab)
	 * { ArrayList<LinkedList<Entity>> row = (ArrayList<LinkedList<Entity>>)
	 * lab.get(indice_i); LinkedList<Entity> elem = row.get(indice_j); return
	 * elem.element(); }
	 * 
	 * 
	 * public void labyrinthe() { for (int i = 0; i < ligne; i++) { for (int j = 0;
	 * j < colonne; j++) { if (tmp[i][j] == -1) { set_element(i, j, new Normal(i, j,
	 * 1, 1, this), labyrinthe); set_element(i, j, new Void(i, j, 1, 1, this),
	 * labyrinthe); set_element(i, j, new Lave(i, j, 1, 1, this), labyrinthe); }
	 * else { set_element(i, j, new Void(i, j, 1, 1, this), labyrinthe);
		...
		for (int i = 0; i < longueur; i++) {
			...
			while (condition == 1 && cases.size() != 0) {
				...
				case 0: // Haut
					x2 = x;
					y2 = y - 1;
					if (y2 > 0 && (get_element(x2, y2, labyrinthe) instanceof Void)) {
						elemxy = get_element(x2, y2, labyrinthe);
					}
					break;
				case 1: // Bas
					x2 = x;
					y2 = y + 1;
					if (y2 < colonne - 1 && (get_element(x2, y2, labyrinthe) instanceof Void)) {
						elemxy = get_element(x2, y2, labyrinthe);
					}
					break;
				case 2: // Gauche
					x2 = x - 1;
					y2 = y;
					if (x2 > 0 && (get_element(x2, y2, labyrinthe) instanceof Void)) {
						elemxy = get_element(x2, y2, labyrinthe);
					}
					break;
				case 3: // Droite
					x2 = x + 1;
					y2 = y;
					if (y2 < ligne - 1 && (get_element(x2, y2, labyrinthe) instanceof Void)) {
						elemxy = get_element(x2, y2, labyrinthe);
					}
					break;
				}
				if (liste.contains(elemxy) || elemxy == null) {
